    ...
    try{
        ...
        const userFollowers = await Promise.all(
            user.followers.map(async(follower)=>{
                const user1 = await User.find({_id: follower});
                console.log("nnnn");
                console.log("usser111: ",user1);
                console.log(user1[0].name+" ",user1[0].username+" ",user1[0].profilePicture);
                return {name: user1[0].name,username: user1[0].username,profilePicture: user1[0].profilePicture}
                
                // return user1.name,user1.username,user1.profilePicture
                // return user1
            })
        )
        // console.log("user : "+user);
...
router.get("/:id/followings",async(req,res)=>{
    ...
    try{
        ...
        const userFollowings = await Promise.all(
            user.isFollowing.map(async(following)=>{
                const user1 = await User.find({_id: following});
                console.log("nnnn");
                console.log("usser111: ",user1);
                console.log(user1[0].name+" ",user1[0].username+" ",user1[0].profilePicture);
                return {name: user1[0].name,username: user1[0].username,profilePicture: user1[0].profilePicture}
